...
def test_ip_restriction_applies_to_admin_only(rf, settings):
    ...
    assert AdminIpRestrictionMiddleware(lambda _: HttpResponse(status=200))(request).status_code == 404
...
def test_ip_restriction_enabled_false(rf, settings):
    ...
    assert AdminIpRestrictionMiddleware(lambda _: HttpResponse(status=200))(request).status_code == 200
...
def test_ip_restriction_missing_x_forwarded_header(rf, settings):
    ...
    assert AdminIpRestrictionMiddleware(lambda _: HttpResponse(status=200))(request).status_code == 401
